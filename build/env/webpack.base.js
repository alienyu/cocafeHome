    ...
    module: {
        rules: [ //加载器，关于各个加载器的参数配置，可自行搜索之。
            ...
            },
            {
                ...
                test: /web.*common.*\.(png|jpg|gif|jpeg|svg)$/,
                loader: `url-loader?limit=8192&name=common/assets/imgs/[hash].[ext]`
            }
        ]
    },
    optimization: {
...
function loadexchangessets(oriConf) {
    // {
    ...
    platformList.map(platform => {
        ...
        pageList.map(page => {
            oriConf.module.rules.push({
                test: new RegExp(`${platform}.*page.*${page}.*\.(png|jpg|gif|jpeg|svg)$`),
                use: `url-loader?limit=8192&name=${platform}/${page}/static/imgs/[hash].[ext]`
            })
        })
    })
    return oriConf;
...
genIconsConfigList = (iconsList) => {
    ...
    uniqueIconsList.map(icon => {
        if(icon.match(/Fill/)) {
            iconsFileContent += `export { default as ${icon} } from '@ant-design/icons/lib/fill/${icon}';\n`
        } else {
...
module.exports = loadexchangessets(baseConf);